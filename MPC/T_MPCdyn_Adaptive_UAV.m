...
bounded = [1.2; -1.2; 1.2; -1.2; 1.2; -1.2; 3; -3];
...
%         args.p(4*i+5:4*i+7)=obs;
    ...
    [vref(:,k),chi_estimados(:,k+1)] = adaptive_OPTI(0*vc(:,k), vc(:,k), v(:,k), chi_estimados(:,k),chi, K1, K2, L, ts);
    
    %% Dinamica del sistema 
    v(:, k+1) = dyn_model_adapUAV(chi_real, v(:,k), vref(:,k), psi(k), L,ts,k);
