...
%% 2) ACELERATIONS VCP 
    ...
    vcp(:,k) = [0;0;0;0];
    %% DYNAMIC ESTIMATION
    [Test(:,k),chi_estimados(:,k+1)] = estimadaptive_dymanic_UAV(chi_estimados(:,k),vcp(:,k), vc(:,k), u_real(:,k), hd(:,k), h(:,k) ,1, L, ts);
    vref(:,k)= vc(:,k)+Test(:,k);
    
    %% Dinamica del sistema 
    [u_real(:, k+1),Tu(:,k)] = dyn_model_adapUAV(chi_real, u_real(:,k), vref(:,k), psi(k), L,ts,k);
